...
def dict_creator(file):
    f = open(file, 'r')
    for line in f:
        # group 1 is id, 2 is race, 3, is name, 4 is rarity
        # 1 RUFS Salvage Corvette (1)
        ship_name = re.search(r'(\d+)\s(\w+)\s(.+)\s\((\d)\)', line)
        if ship_name:
            master_ship_dict[ship_name.group(1)] = [ship_name.group(4), ship_name.group(3), ship_name.group(2)]
    # This is the code block for creating dictionaries for each race, and a master ident/race file.
    # Deprecated as below.
...
"armor_hardpoint": 0, "shield_hardpoint": 0, "engine_hardpoint": 0, "fleet_leader_skill": ""}
    ...
    for ship_id, rare_name_race in master_ship_dict.iteritems():
        ship_attrs = dict(ship_attrs_base.items() + [('name', rare_name_race[1]), ('ident', ship_id), ('rarity', rare_name_race[0]), ('race', rare_name_race[2])])
        ships_dict[ship_id] = ship_attrs
    string = json.dumps(ships_dict)
    ship_file.write(string)


    # This code block is used to generate a different file for each race.  This was deprecated on 2/2/13 when I
    # refactored to use a single ship file that contains the ship race.
